from core.api_clients import HuggingFaceClient
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Format documents according to citation styles using Hugging Face backend.')
...
def main():
    ...
    try:
        ...
        print(f"Using Backend: Hugging Face ({client.model_name})")
        ...
        if not args.report_only:
            ...
            # Pass original path (formatter reloads doc, but we might want to pass the in-memory doc if we fixed errors)
            # The current Core Formatter expects file path.
            # Ideally we should refactor Formatter to accept Doc object, but for now we follow old pattern 
            # OR we save the temporary fixed doc if we fixed errors.
            # The logic in previous app_huggingface.py lines 690+ showed it saves a temp file if fix_errors.
            # But the 'doc' object in memory here has the fixes. 
            # Since Core Formatter loads from path: `doc = Document(input_path)`, we MUST save our in-memory doc to a temp path if we modified it.
